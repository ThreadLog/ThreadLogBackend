...
export const getAllJobsService = async (queryFilters, pagination) => {
    const whereClause = {};
...
export const getJobBankService = async (queryFilters, pagination, searchFilters) => {
    const page = pagination?.page || 1;
    const limit = pagination?.limit || 10;
    ...
    try {
        ...
        if (!response.ok) {
            throw new AppError(`Failed to fetch job bank data: ${response.statusText}`, response.status);
        }
