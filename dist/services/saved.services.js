...
// BOKMÄRK JOBB
// ==========================================
export const saveJobService = async (jobSeekerId, jobId) => {
    if (!jobId || isNaN(jobId)) {
        throw new AppError("Ogiltigt ID. Jobbet du försöker spara verkar inte finnas.", 400);
    }
    const job = await prisma.job.findUnique({ where: { id: jobId } });
    if (!job) {
        throw new AppError("Jobbet du försöker spara finns tyvärr inte längre.", 404);
    }
...
export const unsaveJobService = async (jobSeekerId, jobId) => {
    if (!jobId || isNaN(jobId)) {
...
export const getSavedJobsService = async (jobSeekerId) => {
...
// BOKMÄRK FÖRETAG
// ==========================================
export const saveCompanyService = async (jobSeekerId, companyId) => {
    if (!companyId || isNaN(companyId)) {
        throw new AppError("Ogiltigt ID. Företaget du försöker spara verkar inte finnas.", 400);
    }
...
export const unsaveCompanyService = async (jobSeekerId, companyId) => {
    if (!companyId || isNaN(companyId)) {
...
export const getSavedCompaniesService = async (jobSeekerId) => {
    if (!jobSeekerId || isNaN(jobSeekerId)) {
        throw new AppError("Ogiltigt användar-ID. Vi kunde inte hämta dina sparade företag.", 400);
    }
