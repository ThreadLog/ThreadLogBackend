...
import { saveJobService, unsaveJobService, getSavedJobsService, saveCompanyService, unsaveCompanyService, getSavedCompaniesService, } from "../services/saved.services.js";
// JOBB
...
export const unsaveCompany = async (req, res, next) => {
    try {
        const companyId = Number(req.params.companyId);
        const userId = req.user?.id;
        if (!userId)
...
export const getSavedCompanies = async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return next(new AppError("Unauthorized", 401));
